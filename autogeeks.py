...
st.set_page_config(page_title="AutoGeeks", page_icon="🚗", layout="wide")
...
# ----------------------------
tab1, tab2, tab3, tab4 = st.tabs(["🚗 AutoGeeks", "👥 Nosotros", "❓ FAQ", "🗺️ Roadmap"])

with tab1:
    # ----------------------------
    # PESTAÑA AUTOGEEKS (MODELO PRINCIPAL)
    # ----------------------------
    st.markdown("<h1 style='text-align: center;'>AutoGeeks - Predictor de Precios</h1>", unsafe_allow_html=True)
    ...
    with left_col:
        st.header("Selected Features & Encoding")
    ...
    with right_col:
        ...
        if st.button("Predict Vehicle Price", type="primary", disabled=disabled, use_container_width=True):
            try:
                ...
                st.success(f"${precio_estimado:,.2f}")
    ...
    # ----------------------------
    st.markdown("---")
    st.header("Encoding Information")

    # Mostrar información sobre los diccionarios de encoding
    info_col1, info_col2, info_col3 = st.columns(3)

    with info_col1:
        st.subheader("🏭 Manufacturers")
        st.write(f"**Total:** {len(ManufacturerUnique)} fabricantes")

    with info_col2:
        st.subheader("🎨 Paint Colors")
        st.write(f"**Total:** {len(PaintColorUnique)} colores")

    with info_col3:
        st.subheader("🗺️ States")
        st.write(f"**Total:** {len(StateUnique)} estados")

    # Información sobre las otras categorías
    st.markdown("---")
    ...
    with cat_col1:
        ...
        st.write(f"- **Cylinders:** {len(CylindersUnique)} opciones")
        st.write(f"- **Fuel Types:** {len(FuelUnique)} opciones")

    with cat_col2:
        st.subheader("🚀 Performance")
        st.write(f"- **Transmissions:** {len(TransmissionUnique)} opciones")
        st.write(f"- **Drive Types:** {len(DriveUnique)} opciones")

    with cat_col3:
        st.subheader("📏 Size")
        st.write(f"- **Vehicle Sizes:** {len(SizeUnique)} opciones")

with tab2:
    # ----------------------------
    # PESTAÑA NOSOTROS
    # ----------------------------
    st.header("👥 Conoce al Equipo AutoGeeks")
    
    # Información de los creadores
    creadores = [
        {
            "nombre": "Carlos E Salinas Velasco",
            "rol": "Data Scientist",
            "imagen": "C:\\Users\\Juan\\Desktop\\JCGS\\Prueba_carros_usados_dataset_local\\Photos\\fotocarlos2.jpg",  
            "linkedin": "https://linkedin.com/in/tu-perfil-1",
            "telefono": "+584123205079",
            "correo": "ce.salinasvelasco@gmail.com"
        },
        {
            "nombre": "Juan Carlos Gutierrez", 
            "rol": "Data Scientist",
            "imagen": "C:\\Users\\Juan\\Desktop\\JCGS\\Prueba_carros_usados_dataset_local\\Photos\\fotojuan3.jpg",  
            "linkedin": "https://www.linkedin.com/in/juancgtz/",
            "telefono": "+584120899076",
    ...
    # Crear columnas para cada creador
    cols = st.columns(3)
    ...
    for i, creador in enumerate(creadores):
        with cols[i]:
            ...
            try:
                foto = load_image([creador["imagen"], f"./assets/{creador['imagen']}"])
                if foto:
                    st.image(foto, width=150, use_container_width=True)
                else:
                    st.image(Image.new('RGB', (150, 150), color='gray'), 
                            caption="Foto no disponible", width=150, use_container_width=True)
            except:
                st.image(Image.new('RGB', (150, 150), color='gray'), 
                        caption="Foto no disponible", width=150, use_container_width=True)
            
            # Información personal
            st.subheader(creador["nombre"])
            st.write(f"**{creador['rol']}**")
            
            # Iconos de contacto EN VERTICAL (uno debajo del otro)
            st.markdown("---")
            
            # LinkedIn
            st.markdown(f'🔗 **LinkedIn:**')
            st.markdown(f'[{creador["linkedin"]}]({creador["linkedin"]})')
            
            # Teléfono
            st.markdown(f'📞 **Teléfono:**')
            st.markdown(f'`{creador["telefono"]}`')
            
            # Correo
            st.markdown(f'✉️ **Correo:**')
            st.markdown(f'`{creador["correo"]}`')

    # Texto explicativo sobre el equipo
    st.markdown("---")
    st.subheader("🚀 Sobre Nosotros")
    st.write("""
    **AutoGeeks** es un proyecto desarrollado por un equipo apasionado de profesionales 
    en ciencia de datos e inteligencia artificial. Nuestra misión es proporcionar 
    estimaciones precisas de precios de vehículos usando modelos avanzados de machine learning.
    ...
    Trabajamos con las últimas tecnologías para ofrecerte la mejor experiencia en 
    la estimación de precios de vehículos usados.
    """)
...
with tab3:
    ...
    with st.expander("🤖 ¿Cómo funciona el modelo de predicción?", expanded=True):
        st.write("""
        Nuestro modelo utiliza **XGBoost** (Extreme Gradient Boosting) entrenado con miles de datos históricos 
        de vehículos. Aplicamos técnicas avanzadas como **Target Encoding** para variables categóricas y 
        feature engineering para mejorar la precisión de las predicciones.
        """)
    ...
    with st.expander("🔧 ¿Qué variables considera el modelo?"):
        ...
        - **Características del vehículo:** Año, kilometraje, cilindrada, tipo de combustible
        - **Especificaciones técnicas:** Transmisión, tracción, tamaño
        - **Ubicación y demografía:** Estado donde se vende el vehículo
        - **Apariencia:** Color y fabricante
        """)
    
    with st.expander("💰 ¿Por qué el precio predicho puede diferir del precio real?"):
        st.write("""
        Varios factores pueden afectar la diferencia:
        - Condición específica del vehículo no capturada en los datos
        - Cantidad de datos que se uso para predecir el modelo
        - Factores de mercado locales y temporales
        - Negociación entre comprador y vendedor
        """)
    
    with st.expander("🔄 ¿Con qué frecuencia actualizan el modelo?"):
        st.write("""
        Actualizamos nuestro modelo **semanalmente** con nuevos datos del mercado automotriz. A traves de un data scrapping
        Esto nos permite mantener la precisión y adaptarnos a las tendencias cambiantes del mercado.
        """)
...
with tab4:
    ...
    # PESTAÑA ROADMAP
    # ----------------------------
    st.header("🗺️ Roadmap - Próximos Pasos")
    
    st.subheader("🚀 Fase 1 - Actual (Completado)")
    col1, col2 = st.columns(2)
    ...
    st.markdown("---")
    st.subheader("🔮 Fase 2 - Próximos 3 meses")
    ...
    with roadmap_col2:
        ...
        **🎨 Mejoras de UX/UI:**
        ...
        - Comparación entre múltiples vehículos
        - Modo oscuro/claro
        - Exportación de resultados
        """)
    
    st.markdown("---")
    st.subheader("🌟 Fase 3 - Visión a Largo Plazo")
    
    st.info("""
    **🚀 Características Futuras:**
    - **API pública** para desarrolladores
    - **App móvil nativa** (iOS y Android)
    - **Análisis de imágenes** del vehículo
    - **Recomendaciones inteligentes** de compra/venta
    - **Alertas de mercado** en tiempo real
    - **Integración con plataformas** de venta de autos
    - **Modelos específicos** por tipo de vehículo
    """)
    ...
    with metric_col1:
        st.metric("Precisión Objetivo", "R² > 0.90", "5% mejora")
    
    with metric_col2:
        st.metric("Tiempo Respuesta", "< 1 segundo", "50% mejora")
